  ...
  return (
    ...
    >
      <div className="bg-white flex items-center rounded-full shadow-xl w-full lg:pl-4">
        ...
        <div className="p-3">
          <button
            onClick={() => handleGetUsers(inputValue)}
...
const Users = () => {
  ...
  const handleFollowUser = async ({ cookies, id }) => {
    const data = await followUser({ cookies, id });
    setUserInfo({ ...userInfo, following: data.following });
  };

  const handleUnfollowUser = async ({ cookies, id }) => {
    const data = await unfollowUser({ cookies, id });
    setUserInfo({ ...userInfo, following: data.following });
  };
          ...
          users.map((user) => (
            ...
            >
              ...
              {followingUserIds.includes(user.id) ? (
                <button
                  onClick={() =>
                    handleUnfollowUser({ cookies: userInfo, id: user.id })
                  }
                  class="bg-red-500 hover:opacity-75 text-white rounded-full px-8 py-2 cursor-pointer"
                >
                  解除
                </button>
              ) : (
                <button
                  onClick={() =>
                    handleFollowUser({ cookies: userInfo, id: user.id })
                  }
                  class="bg-blue-500 hover:opacity-75 text-white rounded-full px-8 py-2 cursor-pointer"
                >
                  Follow
                </button>
              )}
