...
const Users = (props) => {
  ...
  const SearchBox = () => {
    return (
      <div className="lg:w-3/6 md:w-5/6 w-full flex items-center justify-center" style={{height: "25%"}}>
        <div class="bg-white flex items-center rounded-full shadow-xl w-full lg:pl-4">
          <input class="input rounded-l-full w-full py-4 px-6 text-gray-700 leading-tight focus:outline-none" id="search" type="text" placeholder="ユーザーを検索する...." />
          <div class="p-3">
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center bg-gradient-to-r from-teal-400 to-blue-500">
      <SearchBox />
      <div className="lg:w-3/6 md:w-5/6 w-full bg-white rounded shadow-xl overflow-y-scroll" style={{height: "60%"}}>

        {users && users.map((user) => (
          <div key={user.id} className="rounded-t flex justify-between items-center w-full border-b-2 border-gray-200 p-6" style={{height: "10vh"}}>
            <div class="flex">
                <div class="mr-4">
                ...
                <div>
                  <h1 class="text-xl font-medium text-gray-700">{user.display_name}</h1>
